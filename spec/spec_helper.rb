...
require 'support/docker/remote/fake_client'
require 'support/docker/fake_cli'
...
module SpecHelpers
  ...
  let(:docker_username)  { 'foo' }
  let(:docker_password)  { 'bar' }
  let(:docker_email)     { 'foo@bar.com' }
  let(:docker_image_url) { 'foo/testapp' }
  let(:docker_cli)       { Docker::FakeCLI.new }
  let(:docker_remote_client) do
    Docker::Remote::FakeClient.new(
      ...
      environment(:production) do
        docker do
          credentials do
            ...
            password context.docker_password
            email context.docker_email
          end
        ...
        kubernetes do
          ...
          add_plugin :rails_app do
            root File.expand_path(File.join('.', 'dummy'), __dir__)
          end
        end
      end
...
RSpec.configure do |config|
  ...
  config.before(:each) do
    Kuby.instance_variable_set(:@definition, nil)
  end
end
